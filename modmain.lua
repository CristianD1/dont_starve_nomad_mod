...
-- Custom items
STRINGS.NAMES.PERSONAL_CHESTER = "Personal Cart"
STRINGS.NAMES.PERSONAL_CHESTER_EYEBONE = "Personal Cart Handle"
...
local function GiveEyebone(inst)
	local eyebone = GLOBAL.SpawnPrefab("personal_cart_handle")
	if eyebone then
		eyebone.owner = inst
		...
		eyebone.components.named:SetName(inst.name.."'s cart handle")
...
local function PersonalCart(inst)
	...
	local OnDespawn_new = function(inst)
		...
		if inst.eyebone then
			...
			print("Error: Player has no linked handle!")
		end
	...
	local OnLoad_new = function(inst, data, newents)
		...
		if data.eyebone ~= nil then
			...
			print("Warning: No handle was loaded from save file!")
		end
...
GLOBAL.c_returneyebone = function(inst)
	...
	if not inst or not inst.ReturnEyebone then 
		print("Error: Cannot return handle")
		return 
	end
	inst:ReturnEyebone()
end

AddPlayerPostInit(PersonalCart)